...
//----------------------------------------------------------------------
        ...
        $discount = (DB::table('discount_code')
            ->where(['code' => $order['code']])->first())->value;
...
//-------------------------------------------------------------------------------
        ...
        if ($order && $order_detail){
            ...
            $order_email->gb_discount = isset($order["gb_discount"]) ? $order["gb_discount"] : null;
            ...
            if ($order_email->gb_discount && $order_email->gb_discount > 0){
			...
			$sent_sms = $this->sendSMSEmailReminder('new_short', app('Dingo\Api\Auth\Auth')->user(), $another_order_email, $order_email->restaurant,
                $order_email->restaurant->lang, $orders_detail_filtered, 'admin');
    ...
    public function getSumPrice(){
        ...
        $clientId = $user->client->ID;
        $sum_price_cs = 0;
        $sum_price_for_bonus_cs = 0;
        $sum_price_en = 0;
        $sum_price_for_bonus_en = 0;
        ...
        if ($orders){
            foreach ($orders as $order) {
                $orders_detail = OrderDetail::where(["ID_orders" => $order->ID])->get();
                foreach ($orders_detail as $order_detail) {
                    if ($order_detail->status == 0 || $order_detail->status == 1 || $order_detail->status == 2 || $order_detail->status == 4) {
                        $serve_time = new Carbon($order_detail->serve_at);
                        $diffInMinutes = $this->getDiffInTime($serve_time, $percentage_update_time);
                        if ($order_detail->currency == "Kč") {
                            if($diffInMinutes > 0)
                                $sum_price_cs += $order_detail->price;
                            $sum_price_for_bonus_cs += $order_detail->price;
                        } else {
                            if($diffInMinutes > 0)
                                $sum_price_en += $order_detail->price;
                            $sum_price_for_bonus_en += $order_detail->price;
                        }
                    }
                }
            }
        ...
        $nBonusQuiz_cs = 0;
        $nBonusQuiz_en = 0;
        $quizClientforBonus = QuizClient::where(["ID_client" => $clientId])->get();
        if($quizClientforBonus) {
            foreach($quizClientforBonus as $quizBonus) {
                if ($quizBonus->lang == "CZE") {
                    $nBonusQuiz_cs += $quizBonus->bonus;
                } else {
                    $nBonusQuiz_en += $quizBonus->bonus;
                }
            }
        }
        return [
            'cs' => [
                'sum_price' => $sum_price_cs,
                'sum_price_for_bonus' => $sum_price_for_bonus_cs,
                'n_bonus_quiz_count' => $nBonusQuiz_cs
            ],
            'en' => [
                'sum_price' => $sum_price_en,
                'sum_price_for_bonus' => $sum_price_for_bonus_en,
                'n_bonus_quiz_count' => $nBonusQuiz_en
            ]
        ];
    }
    ...
    public function sendEmailReminder($type, User $user, Order $order, $restaurant, $lang, $orders_detail_filtered, $to)
    {
        ...
        if ($to == 'user' && $type == 'new') {
            ...
            $message = $this->getEmailSubject($order, $restaurant->lang);
        }
    ...
    function getEmailSubject($order, $lang) {
        $subject = array("pickUp" => array("ENG" => "Gastro Booking - Pick up", "CZE" => "Vyzvednutí Gastro-Booking"),
                     "delivery" => array("ENG" => "Gastro Booking - Delivery", "CZE" => "Dovoz Gastro-Booking"),
                     "order" => array("ENG" => "Gastro Booking - Order", "CZE" => "Objednávka Gastro-Booking"));

        if ($lang == "ENG" || $lang == "en") {
            $subjectLanguage = "ENG";
        }
        else if ($lang == "cs" || $lang == "cz" || $lang == "CZE" ) {
            $subjectLanguage = "CZE";
        }
        else {
            $subjectLanguage = "ENG";
        ...
        return $subject[$orderType][$subjectLanguage];
    }

    function getEmailPath($lang) {
        if ($lang == "ENG" || $lang == "en") {
            return "en";
        }
        else if ($lang == "cs" || $lang == "cz" || $lang == "CZE" ) {
