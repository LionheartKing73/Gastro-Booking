...
class MenuListTransformer extends TransformerAbstract
{
    ...
    public function transform(MenuList $menuList)
    {
        ...
        return [
            ...
            "pick_up" => $menuList->pick_up,
            "client_percentage" => $menuList->client_percentage,
            "ordered" => $this->menuListRepository->isOrdered($menuList) ? (int)$this->menuListRepository->isOrdered($menuList) : 0,
            "total_orders" => $menuList->orders_detail->count(),
            "recommended_side_dishes" => $menuList->recommended_side_dishes,
            "loading" => 0,
            "prefix" => $menuList->prefix,
            'photo' => $menuList->photo ? 'uploads/items/' . $menuList->photo : "",
