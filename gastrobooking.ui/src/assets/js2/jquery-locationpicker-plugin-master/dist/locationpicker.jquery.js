...
(function ($) {
    ...
    function setupInputListenersInput(inputBinding, gmapContext) {
        ...
        if (inputBinding) {
            ...
            if (inputBinding.locationNameInput && gmapContext.settings.enableAutocomplete) {
                ...
                inputBinding.locationNameInput.on("keyup", function (e) {
                    if (e.keyCode == 13) {
                        searchPlaces(inputBinding, gmapContext);

                        if (searchInit) {
                            inputBinding.locationNameInput.closest("form").submit()
                        } else {
                            searchInit = true;
                        }
                    } else {
    ...
    function searchPlaces(inputBinding, gmapContext) {

        var geocoder = new google.maps.Geocoder();

        geocoder.geocode({"address": inputBinding.locationNameInput.val()}, function (results, status) {
            GmUtility.setPosition(gmapContext, results[0].geometry.location, function (context) {
                updateInputValues(inputBinding, context);
                context.settings.onchanged.apply(gmapContext.domContainer, [GmUtility.locationFromLatLng(context.location), context.radius, false]);
            });
    ...
    $.fn.locationPickerFirstAutocomplete = function (options) {
        var _targetDomElement = this.get(0);
        if (!isPluginApplied(_targetDomElement)) return;
        var gmapContext = getContextForElement(_targetDomElement);

        searchPlaces(options.inputBinding, gmapContext);
    }
